...
const defailtZoom = {
  data: {
    columns: [['Nhiệt độ'], ['Độ ẩm'], ['Chất lượng không khí']],
    colors: {
  ...
  useEffect(() => {
    ...
    socket.on('chart_sensor', data => {
      ...
      if (zoom.data.columns[0].length > 1800) {
        const newData = zoom
        newData.data.columns[0] = ['Nhiệt độ', dataSocket.data1]
        setZoom(newData)
      } else {
        const newData = zoom
      ...
      if (zoom.data.columns[1].length > 1800) {
        const newData = zoom
        newData.data.columns[1] = ['Độ ẩm', dataSocket.data2]
        setZoom(newData)
      } else {
        const newData = zoom
      ...
      if (zoom.data.columns[2].length > 1800) {
        const newData = zoom
        newData.data.columns[2] = ['Chất lượng không khí', dataSocket.data3]
        setZoom(newData)
      } else {
        const newData = zoom
